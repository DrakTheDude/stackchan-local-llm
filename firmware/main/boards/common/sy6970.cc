...
int Sy6970::GetBatteryLevel() {
    int level = 0;
    // lowest voltage the battery may discharge to
    ...
    // with no battery connected the charge status reads unreliably, and battery_voltage sometimes exceeds charge_voltage_limit
    if (level > 100) {
        level = 100;
    }
    return level;
