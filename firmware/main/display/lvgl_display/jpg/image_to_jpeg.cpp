...
static uint8_t* convert_input_to_encoder_buf(const uint8_t* src, uint16_t width, uint16_t height, v4l2_pix_fmt_t format,
    ...
    // GRAY goes in directly as JPEG_PIXEL_FORMAT_GRAY
    ...
    // V4L2 YUYV (Y Cb Y Cr) can go in directly as JPEG_PIXEL_FORMAT_YCbYCr
    ...
    // V4L2 UYVY (Cb Y Cr Y) -> reorder to YUYV, then input as YCbYCr
    // UYVY does not occur in the current version
    ...
    // YUV422P does not occur in the current version
    ...
    // RGB is converted to YUV422 (YCbYCr) before input
    // see https://github.com/78/xiaozhi-esp32/issues/1380#issuecomment-3497156378
        ...
        switch (format) {
            ...
            case V4L2_PIX_FMT_RGB565:
                ...
                break;
            [[unlikely]] case V4L2_PIX_FMT_RGB565X: // RGB565X does not occur in the current version
...
static uint8_t* convert_input_to_hw_encoder_buf(const uint8_t* src, uint16_t width, uint16_t height, v4l2_pix_fmt_t format,
    ...
    if (format == V4L2_PIX_FMT_YUYV) {
        // the hardware expects the "big-endian" | Y1 V Y0 U | layout, hence bswap16
        int sz = (int)width * (int)height * 2;
...
static bool encode_with_esp_new_jpeg(const uint8_t* src, size_t src_len, uint16_t width, uint16_t height,
    ...
    // estimate the output buffer: 1.5 x width x height + 64 KB
    size_t out_cap = (size_t)width * (size_t)height * 3 / 2 + 64 * 1024;
    if (out_cap < 128 * 1024)
        out_cap = 128 * 1024;
    ...
    if (cb) {
        cb(cb_arg, 0, outbuf, (size_t)out_len);
        cb(cb_arg, 1, NULL, 0);  // end-of-stream signal
        free(outbuf);
        if (jpg_out)
            *jpg_out = NULL;
        if (jpg_out_len)
